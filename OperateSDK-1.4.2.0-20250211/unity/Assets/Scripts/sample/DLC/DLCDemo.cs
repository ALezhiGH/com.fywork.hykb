...
public class DLCDemo : MonoBehaviour
{
    ...
    public void DoQueryAllDLC()
    {
        HykbDLCQueryAllListenerProxy proxy = new HykbDLCQueryAllListenerProxy(this);
        HykbDLC.queryAll(proxy);
    }

    
    public void DoExchangeDLC()
    {
        HykbDLCExchangeListenerProxy proxy = new HykbDLCExchangeListenerProxy(this);
        HykbDLC.exchange(1, "k1139fc5c6865c33", proxy);
    }
    ...
    private class HykbDLCQueryAllListenerProxy : HykbDLCQueryAllListener
    {
        private DLCDemo instance;

        public HykbDLCQueryAllListenerProxy(DLCDemo instance)
        {
            this.instance = instance;
        }

        public override void OnSucceed(HykbDLCSkuStatusData data) 
        {
            string content = "ok;";
            foreach (HykbDLCSkuStatusInfo item in data.getList()) {
                content = content + ",[skuId:"+item.getSkuId()+",status:"+item.getStatus()+"]";
            }
            instance.ShowText.text = content;
        }
    ...
    private class HykbDLCExchangeListenerProxy : HykbDLCExchangeListener
    {
        private DLCDemo instance;

        public HykbDLCExchangeListenerProxy(DLCDemo instance)
        {
            this.instance = instance;
        }
        ...
        public override void OnSucceed(int ok)
        {
            instance.ShowText.text = "兑换状态:" + ok;
        }
